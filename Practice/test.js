...
var tests1 = require('./Practice1/test-data.json');
...
describe("generate practice test", function() {
    ...
    const result = generatePractice(length,"./Practice3/test-data.json");
    ...
    if (result.length > 5) {
      it(`generated data must contain an input empty array`, function() {
        expect(result.find(e => !e.input.length)).not.toBeUndefined();
      });
    ...
    it(`generated output data must equal to index of target`, function() {
      result.forEach(item => {
        expect(searchPractice(item.input,item.target)).toEqual(item.output);
      });
    });
